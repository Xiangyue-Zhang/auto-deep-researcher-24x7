...
- **Idea Agent**: 文献調査・仮説形成（5ツール）
- **Code Agent**: 実験実装・実行（7ツール）
- **Writing Agent**: レポート・論文執筆（4ツール）

同時に稼働するWorkerは1つだけ。他はゼロトークンコスト。

### v2.0 新機能（2026-06-01）

- **コード理解ツール**: `search_code`（正規表現でワークスペース全体を検索）、`list_tree`（再帰的なディレクトリツリー）、`read_file` の行範囲読み取り（大きなファイルが切り捨てられない）。
- **文献ツール**: `get_paper`（論文詳細＋参照/被引用のスノーボール）、`search_arxiv`（最新プレプリント）。
- **自律レイヤー（追加的・デフォルト動作を変えない）**: 実験台帳 `experiments.jsonl`・停滞検出・DEAD_ENDS/INSIGHTS ジャーナル・違反スキャン・フェーズゲート・バーンアウト防止のレート制限（`max_cycles_per_hour`）。
- すべての信号は THINK/REFLECT の計画コンテキストに注入。ローカルと SSH で同一動作。GPU/ネットワーク不要でユニットテスト可能。
